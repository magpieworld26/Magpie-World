    ...
    <div
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.boxShadow =
          "0 16px 40px rgba(0,0,0,0.5),0 0 20px rgba(0,229,200,0.1)";
      }}
      onMouseLeave={(e) => {
...
export default function HomePage() {
  ...
  const GENRE_BUCKETS: { label: string; match: (g: string) => boolean }[] = [
    { label: "Fantasy", match: (g) => /fantasy|magic|dragon/i.test(g) },
    {
      ...
      match: (g) => /mystery|thriller|detective|crime|noir/i.test(g),
    },
    {
      label: "Sci-Fi",
      match: (g) => /sci-fi|scifi|science fiction|space/i.test(g),
    },
    { label: "Comedy", match: (g) => /comedy|absurd|humou?r|satire/i.test(g) },
    {
      label: "Adventure",
      match: (g) => /adventure|survival|exploration/i.test(g),
    },
    {
      label: "Drama & Literary",
      match: (g) => /drama|literary|historical|slice/i.test(g),
    },
  ];
  ...
  const handleReadStory = async (storyId: string) => {
    const existing = sessions.find(
      (s) => s.storyId === storyId && s.status === "active",
    );
    if (existing) {
        ...
        <div style={{ padding: isMobile ? "0 16px 60px" : "0 4vw 80px" }}>
          ...
          {filteredStories.length === 0 ? (
            ...
            >
              ...
              >
                No stories match "
                <span style={{ color: "rgba(0,229,200,0.7)" }}>
                  {searchQuery.trim()}
                </span>
      ...
      {!q && genreRows.length > 0 && (
        <div style={{ padding: isMobile ? "0 0 20px" : "0 0 20px" }}>
          ...
          {genreRows.map((row) => (
            <div
              key={row.label}
              style={{ padding: isMobile ? "0 16px 32px" : "0 4vw 40px" }}
            >
